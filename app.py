...
app = Flask(__name__)
# Configure CORS to allow all origins. This should be sufficient for development and broad access.
# If you need more restrictive CORS in production, replace "*" with specific origins (e.g., "http://127.0.0.1:5500").
...
"""
        ...
        try:
            ...
            if use_vision_model and not images_b64 and not any('images' in entry for entry in chat_history_raw if entry.get('sender') == 'user'):
                ...
                return jsonify({'answer': f'API Error: {str(req_e)}. Check your API key and permissions.'}), 500
    except Exception as e: 
